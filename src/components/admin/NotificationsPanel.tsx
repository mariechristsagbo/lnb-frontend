...
export const NotificationsPanel: React.FC = () => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        const response = await fetch('http://www.backend.lnb-intranet.globalitnet.org/notifications/received/');
        if (!response.ok) {
                ...
                <div key={notification.id} className="p-4 border rounded-lg">
                  <div className="flex justify-between">
                    ...
                    <span className="px-2 py-1 text-xs rounded-full" 
                      style={{ backgroundColor: notification.notification_type_color + '20', 
                  ...
                  <div className="mt-4 flex justify-between items-center">
                    ...
                    <div className="space-x-2">
                      {!notification.is_read && (
                        <button className="px-3 py-1 border rounded text-sm">Marquer comme lu</button>
                      )}
                      {!notification.is_archived && (
                        <button className="px-3 py-1 border rounded text-sm text-red-500">Archiver</button>
                      )}
                    </div>
                  </div>
                </div>
